...
class SYCLCombinedScheduling(BaseScheduling):
    """
    Scheduler for SYCL Kernels, which delegates calls as appropriate
    to the SYCL-C++ (CUTLASS) and Triton Schedulers, which both work for XPU devices
    ...
    def __init__(self, scheduler: Optional[Scheduler]) -> None:
        ...
        self._sycl_cpp_scheduling = SYCLCPPScheduling(scheduler)
    ...
    def choose_node_backend(self, node: BaseSchedulerNode) -> BaseScheduling:
        if self._sycl_cpp_scheduling.is_sycl_cpp_template(node):
            return self._sycl_cpp_scheduling
        return self._triton_scheduling

    def can_fuse_vertical(
        self, node1: BaseSchedulerNode, node2: BaseSchedulerNode
    ) -> bool:
        if self._sycl_cpp_scheduling.can_fuse_vertical(node1, node2):
            return True
    ...
    def can_fuse_horizontal(
        ...
        for node in (node1, node2):
            if self._sycl_cpp_scheduling.is_sycl_cpp_template(node):
                return self._sycl_cpp_scheduling.can_fuse_horizontal(
                    node1, node2
    ...
    def codegen_template(
        ...
        if self._sycl_cpp_scheduling.is_sycl_cpp_template(template_node):
            assert not epilogue_nodes
            assert not prologue_nodes
            return self._sycl_cpp_scheduling.codegen_template(
