...
from ...ir import SYCLTemplateBuffer
...
class SYCLCPPScheduling(BaseScheduling):
    """
    Partial Scheduling implementation for SYCL C++ Kernels.
    This class is intended to be used in combination with TritonScheduling,
    and delegated to by SYCLCombinedScheduling.

    It handles fusion decisions and SYCL C++ specific template code generation.
    """
    ...
    @staticmethod
    def is_sycl_cpp_template(node: BaseSchedulerNode) -> bool:
        return isinstance(node, SchedulerNode) and isinstance(
            node.node, SYCLTemplateBuffer
        )
    ...
    def define_kernel(self, src_code: str, node_schedule) -> str:
        ...
        if src_code in wrapper.src_to_kernel:
            ...
            kernel_name = "_".join(["sycl", fused_name, wrapper.next_kernel_suffix()])
            ...
            compile_wrapper.writeline("async_compile.sycl(r'''")
    ...
    def codegen_template(
        ...
        assert rnumel == 1
        ctb: SYCLTemplateBuffer = cast(SYCLTemplateBuffer, template_node.node)
